# Disk Usage Analyzer

<div align="center">

**一个强大、快速、跨平台的磁盘空间分析工具**

[![Python Version](https://img.shields.io/badge/python-3.6+-blue.svg)](https://www.python.org/downloads/)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)
[![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey.svg)](https://github.com/yourusername/disk_usage_analyzer)

功能丰富 | 零依赖 | 易于使用

</div>

## ✨ 特性

### 核心功能
- 📊 **快速扫描** - 高效遍历目录树，统计所有文件和文件夹大小
- 🔍 **智能排序** - 自动找出占用空间最大的文件夹和文件
- 📈 **分类统计** - 按文件类型分析空间占用情况
- 🎨 **可视化报告** - 清晰的终端表格输出，带进度条
- 🚀 **缓存机制** - 加速二次扫描，保存扫描结果
- 🔁 **重复文件检测** - 查找重复文件，节省磁盘空间
- 📊 **CSV/JSON导出** - 导出详细报告用于深入分析
- ⚡ **跨平台** - Windows / macOS / Linux 通用

### 辅助工具
- 🦀 **Rust清理工具** - 一键清理Rust编译产物（target目录）
- 🐧 **WSL分析工具** - 深入分析WSL和Docker磁盘占用
- 📦 **批处理脚本** - Windows用户快速分析工具

## 📦 安装

### 要求
- Python 3.6 或更高版本
- 无需额外依赖（仅使用Python标准库）

### 克隆项目

```bash
git clone https://github.com/yourusername/disk_usage_analyzer.git
cd disk_usage_analyzer
```

### 验证安装

```bash
python main.py --help
```

## 🚀 快速开始

### 基本用法

```bash
# 分析当前目录
python main.py

# 分析指定目录（Windows）
python main.py C:\

# 分析指定目录（Linux/macOS）
python main.py /home/user

# 显示前20个结果
python main.py -n 20

# 不显示进度条（更快的输出）
python main.py --no-progress
```

### 高级用法

#### 1. 使用缓存加速

```bash
# 第一次扫描会保存缓存
python main.py C:\

# 第二次使用缓存，几乎瞬间完成
python main.py C:\ --cache

# 强制重新扫描
python main.py C:\ --no-cache
```

#### 2. 查找重复文件

```bash
# 查找重复文件（默认大于1MB）
python main.py ~/Downloads --duplicates

# 只检测大于10MB的文件
python main.py ~/Downloads --duplicates --dup-min-size 10

# 查找所有重复文件
python main.py ~/Documents --duplicates --dup-min-size 0
```

#### 3. 导出报告

```bash
# 导出CSV格式（方便Excel打开）
python main.py C:\ --export-csv

# 导出JSON格式
python main.py C:\ --export-json report.json

# 指定导出目录
python main.py C:\ --export-csv --export-dir ./reports
```

#### 4. 组合使用

```bash
# 分析整个D盘，使用缓存，查找重复文件，导出报告
python main.py D:\ --cache --duplicates --export-csv --export-json d_drive_report.json

# 快速查看Top 20大文件
python main.py . -n 20 --no-progress
```

## 🛠️ 辅助工具

### 1. Rust清理工具

自动查找并清理Rust项目的编译产物（target目录）：

```bash
# 预览模式（查看会删除什么）
python tools/clean_rust.py ~/code

# 执行清理（需要确认）
python tools/clean_rust.py ~/code --execute

# 自动确认，跳过提示
python tools/clean_rust.py ~/code --execute --yes
```

### 2. WSL分析工具

分析WSL和Docker的磁盘占用（仅Windows）：

```bash
# 分析WSL Ubuntu和Docker
python tools/analyze_wsl.py

# 只分析Ubuntu
python tools/analyze_wsl.py --ubuntu

# 只分析Docker
python tools/analyze_wsl.py --docker

# 显示清理命令
python tools/analyze_wsl.py --cleanup
```

### 3. 批处理脚本（Windows）

直接双击运行 `tools/analyze_wsl.bat` 进行快速分析。

## 📊 输出示例

### 基本报告

```
======================================================================
📊 磁盘使用分析报告 - C:\Users\username
======================================================================

📈 总览:
  总大小: 800.9 GB
  文件数: 2,116,211
  目录数: 333,668

📁 最大的文件夹 (Top 10):
----------------------------------------------------------------------
   1. Program Files/Steam/games                 138.3 GB  17.3% ███░░░░
   2. AppData/Local/Packages/Ubuntu              151.3 GB  18.9% ███░░░░
   3. AppData/Local/Docker                      100.4 GB  12.5% ██░░░░░░
   4. Downloads                                  16.8 GB   2.1% ░░░░░░░░
   ...

📄 最大的文件 (Top 10):
----------------------------------------------------------------------
   1. AppData/.../ext4.vhdx                               151.3 GB
   2. Program Files/.../docker_data.vhdx                  100.4 GB
   3. pagefile.sys                                          22.0 GB
   ...

📊 文件类型统计 (Top 10):
----------------------------------------------------------------------
   1. .vhdx                                    252.4 GB  31.5% ██████
   2. .pak                                     141.2 GB  17.6% ███
   3. .dll                                      69.6 GB   8.7% ██
   ...
```

### 重复文件报告

```
🔁 重复文件检测 (Top 10)
======================================================================

📌 重复组 #1 (3 个文件, 各 1.2 GB)
   ⚠️  可节省: 2.4 GB
      1. Downloads/backup.zip
      2. Documents/backup copy.zip
      3. .temp/backup.zip

📌 重复组 #2 (2 个文件, 各 450 MB)
   ⚠️  可节省: 450 MB
      1. Videos/movie.mp4
      2. Backup/movie.mp4

----------------------------------------------------------------------
💰 总计可节省空间: 2.9 GB
======================================================================
```

## 📁 项目结构

```
disk_usage_analyzer/
├── disk_analyzer.py          # 核心分析引擎
├── reporter.py               # 报告生成器
├── main.py                   # CLI 入口
├── tools/                    # 辅助工具
│   ├── __init__.py
│   ├── clean_rust.py         # Rust清理工具
│   ├── analyze_wsl.py        # WSL分析工具
│   ├── analyze_wsl_simple.py # 简化版WSL工具
│   └── analyze_wsl.bat       # Windows批处理脚本
├── .gitignore                # Git忽略配置
├── README.md                 # 项目文档
└── LICENSE                   # MIT许可证
```

## 🔧 技术实现

### 核心技术
- **扫描引擎**: 使用 `os.walk()` 高效遍历目录树
- **缓存机制**: 使用 `pickle` 序列化扫描结果，基于路径哈希存储
- **重复检测**: 先按文件大小预筛选，再计算 MD5 哈希确认
- **导出功能**:
  - CSV: 使用 `csv` 模块（UTF-8-BOM编码，Excel兼容）
  - JSON: 使用 `json` 模块，包含完整统计信息

### 性能优化
- 仅在内存中保留必要信息（路径、大小、修改时间）
- 使用生成器处理大型目录
- 智能缓存避免重复扫描
- MD5计算仅在需要时进行

## 🛣️ 开发路线

### 已完成 (v1.1)
- [x] 基础扫描和统计功能
- [x] 终端可视化报告
- [x] CSV/JSON 导出
- [x] 重复文件检测
- [x] 缓存机制加速二次扫描
- [x] Rust清理工具
- [x] WSL分析工具

### 计划中
- [ ] HTML可视化报告
- [ ] 忽略列表配置文件
- [ ] 定时扫描和报告
- [ ] Web界面
- [ ] 实时监控模式
- [ ] 云存储支持
- [ ] 文件年龄分析

## 🤝 贡献

欢迎贡献！请随时提交问题或拉取请求。

1. Fork 本项目
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启拉取请求

## 📝 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

## 👨‍💻 作者

**Your Name** - [@yourusername](https://github.com/yourusername)

## 🙏 致谢

- 灵感来源于 [ncdu](https://dev.yorhel.nl/ncdu) 和 [WinDirStat](https://windirstat.net/)
- 感谢所有贡献者

## 📧 联系方式

- GitHub Issues: [提交问题](https://github.com/yourusername/disk_usage_analyzer/issues)
- Email: your.email@example.com

---

<div align="center">

**如果这个项目对你有帮助，请给它一个⭐️**

Made with ❤️ by [Your Name](https://github.com/yourusername)

</div>
